...
// Vercel cron entry point. Verifies the cron secret, then proxies to the
// FastAPI backend which performs the actual work. The backend job needs a
// JWT bound to a privileged user — supplied via CRON_USER_ID env var.
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await backendFetch("/api/hira/cron/review-scheduler", {
      method: "POST",
      userId: cronUserId
    });
