// Two-phase upload for Near Miss attachments. Mirrors the Observation
// upload helper — same init / PUT / complete pattern, different endpoint.

export type NearMissUploadCategory =
  | "INITIAL_PHOTO"
  | "WITNESS_STATEMENT"
  | "EVIDENCE"
  | "CAPA_EVIDENCE"
...
export async function uploadNearMissAttachment(
  nearMissId: string,
  file: File,
  category: NearMissUploadCategory,
  caption?: string
): Promise<UploadResult> {
  try {
    const exif = await extractExif(file);

    const initRes = await fetch(`/api/near-miss/${nearMissId}/attachments`, {
      method: "POST",
    ...
    const completeRes = await fetch(`/api/near-miss/${nearMissId}/attachments`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        phase: "complete",
