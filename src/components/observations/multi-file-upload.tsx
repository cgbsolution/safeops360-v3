...
// Same lifecycle as the incident MultiFileUpload — see that file's docs.
// Duplicated rather than generalised so changes to either module's upload
// surface stay isolated.
export function ObservationMultiFileUpload({
  observationId,
  category,
  accept = "image/*",
  ...
  category: "INITIAL_PHOTO" | "ACTION_EVIDENCE" | "VERIFICATION_PHOTO" | "DOCUMENT";
  accept?: string;
  buttonLabel?: string;
  helpText?: string;
  ...
  async function saveCaption(local: LocalFile, caption: string) {
    ...
    try {
      await fetch(`/api/observations/${observationId}/attachments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          phase: "complete",
      ...
      /* swallow — caption is non-critical */
    }
  }
  ...
  async function removeOne(local: LocalFile) {
    ...
    if (local.attachmentId) {
      try {
        await fetch(`/api/observations/${observationId}/attachments/${local.attachmentId}`, {
          method: "DELETE"
        });
      } catch {
        /* server-side delete failure isn't fatal */
      }
    }
  }
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <p className="text-[11px] text-slate-500 mt-2">
          Up to {MAX_FILES} files · {Math.round(MAX_SIZE / 1024 / 1024)} MB each · JPG / PNG / HEIC / MP4 / PDF
        </p>
      </div>
